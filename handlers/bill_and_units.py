...
def register_bill_and_units(bot, history):
    ...
    def mtn_unit_final_confirm(call):
        ...
        # 1) أضف الطلب إلى طابور queue_service
        add_pending_request(
            user_id=user_id,
            username=call.from_user.username,
            request_text=(
        ...
        # 5) أرسل الملخص للمدير مع أزرار الموافقة/الرفض
        bot.send_message(
            ADMIN_MAIN_ID,
            summary,
            reply_markup=kb_admin
...
def register(bot):
    """
    هذه الدالة تستدعى من main.py فقط للتماشي مع الواجهة
    جميع الـ handlers مسجّلة بالفعل بالديكوريتورات عند استيراد الملف.
    """
    pass
